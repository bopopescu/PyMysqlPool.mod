# Dynamic Pool MySQL Connector/Python - MySQL driver written in Python.
...
class MySQLConnectionPool(object):
    ...
    def __init__(self, pool_size=0, pool_name=None, pool_reset_session=True,
        ...
        instances.
        """
        self._pool_size = 0   # None
        self._dynamic_size = pool_size
        self._cnx_queue = queue.Queue(self._pool_size)
        self._check_size = 0  # None
        self._init_size = 0   # None
        self._pool_name = None
        self._increase_size = 0  #None
        self._reset_session = pool_reset_session
        self._set_pool_size(pool_size)
        self._set_init_size(pool_size)
        self._set_check_size(pool_size)
    ...
    def _set_init_size(self, pool_size):
        ...
        if self._init_size == 0:
            self._init_size = 5
            self._pool_size = self._init_size

    def _set_check_size(self, pool_size):
        """Set the size of the pool
    ...
    def dynamic_load(self):
        """load increase connections
            This method increase 2n+1-self._pool_size connections.
            Returns None.
        """
        ...
        if size <= self._check_size:
            ...
            while self._pool_size < self._increase_size:
                self.add_connection()
                self._pool_size += 1
            self._set_check_size(self._increase_size)
